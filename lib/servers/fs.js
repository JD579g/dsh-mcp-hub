...
import { defaultFsRoots, pathPolicy, underPrefix } from '../platform.js'
...
export async function resolveSandboxPath(input, options = {}) {
  ...
  const readOnly = rules.writeBlocked.find((prefix) => underPrefix(absolute, prefix))
  ...
  if (!allowed.some((root) => inside(resolved, root))) {
    throw new Error('路径越界（解析后）：' + resolved + ' 不在允许根目录之内')
  }
