 ...
 * 平台的临时目录（可能不止一个）。
 *
 * 为什么需要这个：stdin MCP 服务经常被**过滤过环境的宿主**拉起
 * （官方 SDK 的 getDefaultEnvironment() 就不传 TMPDIR），子进程里的 os.tmpdir()
 * 于是退化成平台惯例值。macOS 上父进程的临时目录是 /var/folders/...，
 * 子进程却看到 /tmp —— 两边对"临时目录"的理解不一致就会互相拒绝。
 * 这里统一列出该平台所有合理的临时目录，供"默认根目录"和"可写白名单"使用。
 */
export function tempRoots(platform = process.platform, env = process.env) {
  const out = []
  const push = (value) => {
    if (typeof value === 'string' && value !== '' && !out.includes(value)) out.push(value)
  }
  // os.tmpdir() 只对"当前正在跑的这台机器"有意义：拿它去描述别的平台会张冠李戴
  // （比如在 Linux 上测 win32 分支时，它返回的是 Linux 的 /tmp）。
  const sameFamily = familyOf(platform) === familyOf(process.platform)
  if (familyOf(platform) === WINDOWS) {
    push(env.TEMP)
    push(env.TMP)
    if (sameFamily) push(os.tmpdir())
  } else {
    push(env.TMPDIR)
    if (sameFamily) push(os.tmpdir())
    // POSIX 惯例：/tmp 永远是临时目录（macOS 上 /var/folders 才是默认，但 /tmp 依然可写）
    push('/tmp')
  }
  return out
}

/** 路径是不是落在某个临时目录里（写策略据此放行）。 */
export function isTempPath(absolute, platform = process.platform, env = process.env) {
  // 用目标平台的 path 实现来归一化：在 Linux 上测 win32 分支时，
  // path.resolve('C:/x') 会把它当成相对路径拼到 cwd 上，结论就全错了。
  const resolved = pathApi(platform).resolve(absolute)
  return tempRoots(platform, env).some((root) => underPrefix(resolved, pathApi(platform).resolve(root), platform))
}

export function defaultFsRoots(platform = process.platform, home = os.homedir(), tmp = os.tmpdir()) {
  // 除了传入的 home/tmp，再带上平台惯例临时目录：宿主可能把 TMPDIR 过滤掉了
  // （官方 SDK 就是），这时子进程里的 os.tmpdir() 会退化成 /tmp 之类 —— 两边都得认。
  return dedupe([home, tmp, ...tempRoots(platform)], platform)
}
